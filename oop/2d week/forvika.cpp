...
class ZeroHeight : public Exception 
{
protected:
    int rows, cols;
public:
    ZeroHeight(const char* s, int Rows, int Columns) : Exception(s) { rows = Rows; cols = Columns; }
    ZeroHeight(const ZeroHeight& e) : Exception(e) { rows = e.rows; cols = e.cols; }
    virtual void print()
    {
        cout << "ZeroHeight: " << str << "; Rows: " << rows << ", Columns: " << cols << "; " << what();
    }
};
...
public:
	...
	Matrix<T> MyFunction_15()
    {
        ...
        if (H <= 0)
            throw ZeroHeight("Matrix height can't be zero or negative in constructor int, int", H, BaseMatrix<T>::width);
        ...
        for (int i = 0; i < BaseMatrix<T>::height; i++)
        {
            if (h[i] != 1)
            {
                for (int j = 0; j < BaseMatrix<T>::width; j++)
                {
                    res.ptr[cur][j] = BaseMatrix<T>::ptr[i][j];
                }
                cur++;
            }
        }
...
int main()
{
	try
	{
        ...
        M3(3, 3) = 0;
		cout << '\n' << M1 << '\n' << M3;
		Matrix<double> M4 = M3.MyFunction_15(); //функция из варианта (при вызове с параметром sq1 выдаст мою собственную ошибку NonScalarFunctionException)
		cout << '\n' << M4;
